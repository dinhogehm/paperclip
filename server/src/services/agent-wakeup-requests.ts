...
export function agentWakeupRequestService(db: Db) {
  ...
  async function cancel(
    ...
    let wakeupRequest = await getByIdForCompany(id, companyId);
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      ...
      const current = await getByIdForCompany(id, companyId);
